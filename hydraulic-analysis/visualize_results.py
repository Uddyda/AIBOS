...
clear_directory('./tmp/image_results')#ディレクトリの中身を一度削除
# %% % --- % --- % --- % --- % --- % --- % --- % --- % --- % --- % ---
...
for k in range(0,len(df_keys)):
    ...
    seg_name=os.path.splitext(os.path.basename(dyna_file))[0]
    parts_name=seg_name.split('_')   
    INP_FILE = f"./tmp/input_files/{parts_name[1]}.inp" #インプットファイルのパス
    ...
    # カラーバーの追加####
    cb1 = plt.cm.ScalarMappable(cmap="jet", norm=plt.Normalize(vmin=100, vmax=nodes_3857["pressure"].max()))
    ...
    #単位付きカラーバーの追加####
    cb2 = plt.cm.ScalarMappable(cmap="jet", norm=plt.Normalize(vmin=2.0, vmax=edges_3857["flow"].max()))
    ...
    plt.show()
    seg_name=os.path.splitext(os.path.basename(dyna_file))[0]
    output_directory = './tmp/image_results'

    if not os.path.exists(output_directory):#ディレクトリがないときは作成
        os.makedirs(output_directory)

    fig.savefig(f"./tmp/image_results/{seg_name}.png", dpi=300)
    print(f"./tmp/image_results/{seg_name}.png")
